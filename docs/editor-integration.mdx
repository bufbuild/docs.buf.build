...
Our Buf extension currently supports linting. As it doesn't support syntax highlighting, we recommend installing one of the [other available extensions][vs_code_protobuf_extensions] for that functionality.
...
[vs_code_protobuf_extensions]: https://marketplace.visualstudio.com/search?term=protocol%20buffers&target=VSCode&category=All%20categories&sortBy=Relevance